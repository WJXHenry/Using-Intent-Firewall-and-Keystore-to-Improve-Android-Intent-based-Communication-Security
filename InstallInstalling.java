...
public class InstallInstalling extends Activity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        //         .getParcelableExtra(PackageUtil.INTENT_ATTR_APPLICATION_INFO);
        // mPackageURI = getIntent().getData();
        mCallingPackage = getIntent().getStringExtra(Intent.EXTRA_INTENT_FIREWALL);
        // IntentFirewall
    ...
    private void launchSuccess() {
        ...
        // IntentFirewall
        Log.d("IntentFirewall", "PackageInstaller: get installer package name: " + mCallingPackage);
        // Set the action of the intent starting the InstallSuccess activity so that it can
        // be caught by the IntentFirewall class (only if the calling package is FDroid)
        if (mCallingPackage != null) {
            if (mCallingPackage.equals("org.fdroid.fdroid")) {
                Log.d("IntentFirewall", "PackageInstaller: caller was FDroid");
                successIntent.setAction("com.android.packageinstaller.ACTION_INSTALL_SUCCESS");
                try {
                    // Inject package name of app being installed into extra field
                    successIntent.putExtra(Intent.EXTRA_INTENT_FIREWALL, appInfo.packageName);
                } catch (Exception e) {
                    Log.d("IntentFirewall", "PackageInstaller_InstallInstalling: launchSuccess_Error: " + e.getMessage());
                }
            }
        }
        // IntentFirewall
